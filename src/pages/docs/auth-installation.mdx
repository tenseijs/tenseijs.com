...
axios.post('http://localhost:4500/auth/register', {
...
### User Registration
User registration works exactly as with [session-based authentication](#user-registration), but this time the `access_token` and `refresh_token` will be included in the response body.

### User login
User login works exactly as with [session-based authentication](#user-login), but this time the `access_token` and `refresh_token` will be included in the response body.

### Password resets
Password resets work exactly as with [session-based authentication](#password-resets). 
